 ...
 function exportResearchAsset(assetName: string) {
  const matchingChat = projectChats.find((chat) =>
    chat.messages.some((message) =>
      message.text.toLowerCase().includes(assetName.toLowerCase())
    )
  );

  const assistantOutput =
    matchingChat?.messages
...
${
  assistantOutput ||
  "No matching generated content was found in chat history. Please regenerate this asset and export again."
}
`;
  ...
  link.href = url;
  link.download = `${assetName.replace(/\s+/g, "_")}.md`;
...
<button
  onClick={() =>
    exportResearchAsset(asset.label)
  }
  className="mt-3 bg-blue-700 hover:bg-blue-800 rounded-lg px-3 py-1 text-xs font-semibold"
>
  Export Asset
</button>
        </div>
