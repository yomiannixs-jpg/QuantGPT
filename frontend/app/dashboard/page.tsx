...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (savedChats) {
      ...
      if (parsedChats.length > 0) {
        ...
        const activeExists = parsedChats.some(
          (chat) => chat.id === savedActiveChatId
        );

        const selectedChatId = activeExists
          ? savedActiveChatId || parsedChats[0].id
          : parsedChats[0].id;
  ...
  async function sendMessage() {
    ...
    updateActiveChat((chat) => {
      const shouldRename = chat.title === "New Chat";

      return {
        ...chat,
        title: shouldRename ? generateTitle(currentMessage) : chat.title,
        mode,
  ...
  async function uploadFile() {
    ...
    updateActiveChat((chat) => {
      ...
      return {
        ...chat,
        title: shouldRename ? `File: ${selectedFile.name}` : chat.title,
        mode,
